var Template = {
    ...
    initResume: function() {
        ...
        if (!(a.length < 1 || a.data("scrollerInited"))) {
            ...
            var b = this
              , c = a.parents("section").find(".layout-title");
            a.on({
                "show.bs.collapse": function() {
                    if ("fixed" == c.css("position")) {
                        ...
                        setTimeout(function() {
                            var d = c.height();
                            b.layout.scrollTo(a.position().top - d, {
                                scrollInertia: 200
                            })
                        }, 500)
                    }
            ...
            var b = function(b) {
                a.find("input,textarea,select").each(function() {
                    var a = $(this)
                      , d = a.attr("name")
                      , e = a.parent()
                      , f = e.find(".form__item__failed-wrap");
                    if (f.length) {
                        var g = b && b[d] ? b[d] : "";
                        f.text(g),
                        g ? e.addClass(c) : e.removeClass(c)
                    }
                })
            }
              , c = "form__item--invalid"
              , d = "form__status--success"
              , e = "form__status--error"
              , f = a.find(".form__status")
            ...
            a.on("submit", function(c) {
                ...
                jQuery.isEmptyObject(i) && $.ajax({
                    ...
                    complete: function(c, g) {
                        ...
                        if (b(h.errors || {}),
                        "success" == g && h.success)
                            f.addClass(d).text(h.message || "Your message was sent successfully. Thanks."),
                            a.trigger("reset");
                        else {
    ...
    _applyFooterSidebarPatch: function() {
        if (this.tools.isTouch() && /iPad/i.test(navigator.userAgent)) {
            ...
            $(window).on("resize", function() {
                a || (a = !0,
                $(".sidebar .footer").hide().delay(1e3).fadeIn("slow", function() {
                    a = !1
                }))
            })
        }
...
Template.menu = {
    ...
    init: function(a) {
        if (!this._inited) {
            ...
            a && (a.on && ($(this).on(a.on),
            delete a.on),
            $.isEmptyObject(a) || $.extend(this, a)),
            this._initMainMenu(),
            this._initMobileMenu();
            var b = this;
            $(window).on("hashchange", function(a) {
                (!a.fragment || a.fragment.match(/\.html$/)) && b.loadPage(a.fragment)
            })
        }
    },
    ...
    loadPage: function(a) {
        return this.isFileProtocol() ? void (document.location = a) : void $.ajax(a, {
            success: function(a) {
                var b = this.isIe(9)
                  , c = document.createElement(b ? "xhtml" : "html");
                c.innerHTML = a;
                var d = $("head title", c);
                d.length && $("head title").text(d.text());
                var e = this.getMenuItems(!1, c).filter("." + this.currenMenuItemClass).find("a")
                  , f = "flap";
                if (e.length) {
                    var g = this.getMenuItems(!0).filter('[href="' + e.attr("href") + '"]');
                    if (g.length > 0) {
                        var h = this.getMenuItems().filter("." + this.currenMenuItemClass).find("a");
                        h.length && h.attr("href") != g.attr("href") && "fixed" == $(".layout-sidebar").css("position") && (f = h.position().left < g.position().left ? "slideLeft" : "slideRight"),
                        g.trigger("activate")
                    }
                }
                var i = $(c).find("section")
                  , j = $("section");
                $(this).trigger("pageLoaded"),
                this._replaceSection(j, i, f)
            },
            context: this
        })
    },
    ...
    _replaceSection: function(a, b, c) {
        ...
        case "slideRight":
            var f = a.parent()
              , g = a.parents(".layout-container-main");
            g.css({
                height: Math.max(f.height(), $(".sidebar").height())
            });
            var h = "slideLeft" == c ? "right" : "left"
              ...
              , j = i.find(".slot1, .slot2").css({
                width: "50%",
                "float": "left"
            })
              , k = {
                forNew: j.eq("slideLeft" == c ? 0 : 1),
                forOld: j.eq("slideLeft" == c ? 1 : 0)
            }
              , l = {
                duration: d
            };
            ...
            m && i.find(".layout-title").addClass(m),
            i.appendTo(f),
            this._applyContainerBg(e),
            i.animate(l, function() {
                b.appendTo(f),
                i.remove(),
                m && b.find("." + m).removeClass(m),
                g.css({
                    height: "auto"
                }),
                Template.init()
            ...
            break;
        case "flap":
            var f = a.parents(".layout-container-main")
              , n = f.parent().height()
              , o = this;
            f.animate({
                height: 10,
    ...
    _initMainMenu: function() {
        var a = this.getSlider()
          , b = this.getMenuItems()
          , c = this;
        b.on("activate", function(b, d) {
            var e = $(this)
              , f = e.position();
            animDuration = d && d.noAnimation ? 0 : 300,
            newSliderWidth = e.outerWidth() + 2,
            $(c).trigger("itemActivate"),
            animDuration < 1 ? (a.css({
                width: newSliderWidth,
                left: f.left,
                display: "block"
            }),
            c._setActiveItem(e)) : a.width(newSliderWidth).animate({
                left: f.left
            }, {
    ...
    _initMobileMenu: function() {
        var a = this
          ...
          , d = "mobile-menu--open"
          , e = "main-nav--mobile"
          , f = this.getHeaderMenu().clone(!0).attr("class", e).appendTo(b);
        ...
        c.on("click", function(c) {
            ...
            var e = b.hasClass(d);
            e ? (b.removeClass(d),
            f.slideToggle(400)) : ($(a).trigger("mobileMenuOpen"),
            b.addClass(d),
            f.slideToggle(400))
        }),
        $(this).on("mobileMenuOpen", function() {
            var b = new RegExp("(?:#)?([a-zA-Z]*)(?:.)?(?:(?:html)|(?:php))?$","i")
              , c = b.exec(window.location.href)
              , d = $("." + e + " li")
              , f = null ;
            c[1] && (d.removeClass(a.currenMenuItemClass),
            f = d.find("a[href *=" + c[1] + "]").parent(),
            f.length > 0 ? f.addClass(a.currenMenuItemClass) : d.first().addClass(a.currenMenuItemClass))
        })
    },
    _setActiveItem: function(a) {
        this._activeItem = a;
        var b = this.currenMenuItemClass;
        a.is("." + b) || (this.getMenuItems().filter("." + b).removeClass(b),
        a.addClass(b))
    },
    ...
    _restoreActiveCurrentPageState: function() {
        ...
        if (document.location.hash) {
            ...
            document.location.pathname.substr(0 - c.length) != c && (a = b.find('a[href$="' + c + '"]'),
            this.loadPage(c))
        }
        (!a || a.length < 1) && (a = b.filter("." + this.currenMenuItemClass),
        a.length < 1 && (a = b.first()),
        a.trigger("activate", {
            noAnimation: !0
        }))
    },
...
Template.layout = {
    customScrollEnabled: !1,
    scrollContainer: "",
    init: function(a) {
        return this._inited ? !1 : (this._inited = !0,
        a && $.extend(this, a),
        this._initCustomScroll() ? !0 : !1)
    },
...
Template.shortcodes = {
    init: function(a) {
        a || (a = document),
        this.initAccordion(a),
        this.initProgressBar(a, 2e3),
        this.initRating(a),
        this.initDonutChart(a)
    },
    hardReset: function() {
    ...
    initProgressBar: function(a, b) {
        ...
        if (!(c.length < 1)) {
            ...
            var d = this
              ...
              , f = ".progress-bar__bar"
              , g = ".progress-bar__value"
              , h = "%";
            c.each(function() {
                var a = $(this).find(e)
                  , c = $(this).find(f).attr("data-level")
                  , i = c + h;
                b ? $(this).find(g).animate({
                    width: c + "%"
                }, b) : $(this).find(g).css({
                    width: c + "%"
                }),
                d._animateCounterValue(c, i, a, b, h)
            })
        }
    },
    ...
    initRating: function(a) {
        ...
        b.length < 1 || b.data("inited") || (b.data("inited", !0),
        b.raty({
            starType: "i",
            number: 8,
            noRatedMsg: " ",
    ...
    initDonutChart: function(a) {
        ...
        if (!(b.length < 1 || b.data("inited"))) {
            ...
            var c = b.css("color")
              , d = b.find(".donut-chart__pallete-second-color").css("color")
...
Template.tools = {
    isIe: function(a) {
        ...
        if (navigator.userAgent) {
            ...
            if (ieInfo = /(msie) ([\w.]+)/.exec(e.toLowerCase()),
            ieInfo) {
                if (!d)
                    return !0;
                b = !0,
                c = parseInt(ieInfo.pop(), 10)
            } else {
                var f = e.indexOf("Trident/");
                if (f > 0) {
                    if (!d)
                        return !0;
                    b = !0;
                    ...
                    var h = e.indexOf("Edge/");
                    if (h > 0) {
                        if (!d)
                            return !0;
                        b = !0,
                        c = parseInt(e.substring(h + 5, e.indexOf(".", h)), 10)
                    }
                }
            }
            if (b && d)
...
jQuery(Template.menu).on("pageLoaded", function() {
    if (window.ga) {
        var a = "/" + document.location.hash.replace(/^#/, "")
          , b = jQuery("head title").text();
        b && (b = b.replace(/^Savvy - /i, "") + " - Ajax"),
        ga("send", "pageview", {
            page: a,
            title: b
        })
